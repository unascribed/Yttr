		...
		if (!isBlind()) {
			if (player.squaredDistanceTo(getHeadPos()) < 4*4) {
				...
				if (world.raycast(new RaycastContext(head, player.getCameraPosVec(1), ShapeType.COLLIDER, FluidHandling.NONE, player)).getType() == Type.MISS) {
					...
					if (recipe.isPresent()) {
						if (recipe.get().getOutput().getItem() == Yttr.AWARE_HOPPER.asItem()) {
							sayTicks = -60;
							world.playSound(null, pos, Yttr.AWARE_HOPPER_SCREAM, SoundCategory.BLOCKS, 1, (world.random.nextFloat()-world.random.nextFloat())*0.2f + 1);
							return;
						}
