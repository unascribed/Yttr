		...
		if (!bs.isOf(block) && (bs.isAir() || bs.getHardness(world, pos) >= 0)) {
			if (bs.getFluidState().isIn(FluidTags.LAVA)) {
				...
				if (!bs.isAir()) {
					...
					world.playSound(null, pos, Yttr.DISSOLVE, SoundCategory.BLOCKS, 1, 0.8f+(world.random.nextFloat()*0.4f));
				}
				world.setBlockState(pos, block.getDefaultState());
				world.getFluidTickScheduler().schedule(pos, Yttr.VOID, 1);
			}
		}
	}
